...
# Private containers: source PDFs, translated outputs, workspace artifacts
CONTAINER_SOURCE_PDFS="${CONTAINER_SOURCE_PDFS:-source-pdfs}"
CONTAINER_OUTPUT="${CONTAINER_OUTPUT:-output}"
CONTAINER_BOOK_WORKSPACES="${CONTAINER_BOOK_WORKSPACES:-book-workspaces}"
PRIVATE_CONTAINERS=(
    "${CONTAINER_SOURCE_PDFS}"
    "${CONTAINER_OUTPUT}"
    "${CONTAINER_BOOK_WORKSPACES}"
)
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 3: Create private containers — source-pdfs, output, book-workspaces
# ─────────────────────────────────────────────────────────────────────────────
echo "══════════════════════════════════════════════════════════════════════"
echo " Step 3: Create private containers"
echo "══════════════════════════════════════════════════════════════════════"
# --public-access off: no anonymous reads. Internal pipeline artifacts stay
# private; only the Azure Static Website endpoint serves public book downloads.
# --auth-mode login: uses Entra ID / Managed Identity rather than storage keys.
echo ""
for container_name in "${PRIVATE_CONTAINERS[@]}"; do
    echo "  → ${container_name}"
...
# URL pattern: https://<account>.z<n>.web.core.windows.net/<slug>/
#
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    echo "  ${STATIC_URL}{slug}/"
    echo ""
    echo "  Example:"
    echo "  ${STATIC_URL}shiv-sutra/"
    echo ""
    echo "  Add this to local and deployed app configuration:"
    echo "  TRANSPOSE_BLOB_STATIC_WEBSITE_URL=${STATIC_URL}"
fi
echo ""
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    echo ""
    echo "  # (a) Confirm required private containers exist with private access"
...
echo "  2. Set TRANSPOSE_BLOB_STATIC_WEBSITE_URL=${STATIC_URL:-<static-website-url>} in .env and Container App config"
echo "  3. Upload a test PDF: az storage blob upload \\"
echo "       --account-name ${STORAGE_ACCOUNT} \\"
echo "       --container-name ${CONTAINER_BOOK_WORKSPACES} \\"
echo "       --name 'test-book--000000/input/source.pdf' \\"
echo "       --file /path/to/your.pdf \\"
echo "       --auth-mode login"
echo "  4. Run the DB migration: alembic upgrade head"
echo "══════════════════════════════════════════════════════════════════════"
echo ""
