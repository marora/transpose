...
### 2026-05-20T23:19:30-04:00: User directive
**By:** Manish (via Copilot)
**What:** Optimize for wow factor with early users — invest in polish (e.g., landing pages with OG previews from day one) that gets initial readers excited to read and provide feedback. Don't ship a bare-bones MVP when a small additional investment yields a much stronger first impression.
**Why:** User request — captured for team memory. Resolves Niobe-vs-Morpheus disagreement on WhatsApp preview deferral. Sets a project-wide bias.

---

# Tank Decision: Phase 1 Migration — Dedicated Columns vs JSONB-Only

**Author:** Tank (Cloud/Infra Dev)
**Date:** 2026-05-20T23:19:30-04:00
**Relates to:** Morpheus's 2026-05-20T22:52 BookWorkspace schema extension decision

## Decision

Chose **Morpheus Option B** (dedicated columns) for the `books` table schema extension:

```sql
ALTER TABLE books ADD COLUMN license_status TEXT NOT NULL DEFAULT 'rights-unknown'
  CHECK (license_status IN ('rights-unknown','claimed-public-domain','verified-public-domain','rights-cleared'));
ALTER TABLE books ADD COLUMN provenance_source JSONB;
ALTER TABLE books ADD COLUMN license_history JSONB NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE books ADD COLUMN metadata JSONB NOT NULL DEFAULT '{}';
```

`license_status` is a real column (not a JSONB sub-key) because:
1. The promotion gate runs `WHERE license_status IN ('verified-public-domain', 'rights-cleared')` — a real column can be indexed; a JSONB sub-key requires a functional index and `->>` operator.
2. The CHECK constraint is enforceable at the DB layer only on a typed column, not on JSONB values.
3. `pg_constraint` visibility and `\d books` readability are better with a named column.

`license_history` is added as a dedicated JSONB column (append-only array) rather than nested inside `metadata JSONB` so that mutation log queries don't require `jsonb_array_elements` across the full metadata blob.

## Migration file

`migrations/versions/2f8c4a91d3e7_add_license_provenance_metadata_to_books.py`

## Self-test approach

Embedded a `DO $$` block inside `upgrade()` that inserts an invalid `license_status` and asserts a `check_violation` exception is raised. Failure means the constraint is absent — the migration itself fails rather than silently deploying a broken schema.

---

# Spec Gap: Twitter Card Meta Tags Missing from Landing Page Template

**From:** Dozer (Tester)  
**To:** Morpheus (Architect)  
**Date:** 2026-05-20T23:30:00-04:00  
**Priority:** Should-fix before Trinity TR-3 merges

---

## The Gap

The landing page HTML template in `decisions.md §C` (Architecture Addendum: Share URL + WhatsApp Preview) does **not** include `<meta name="twitter:card">` tags.

The D-2 test spec I was given requires testing for Twitter Card meta tags, and Dozer's golden fixture (`tests/golden/landing_page_fixture.html`) asserts their presence:
...
- `tests/golden/landing_page_fixture.html` — includes Twitter Card tags (as a test assertion target)
- `tests/unit/pipeline/test_landing_page.py::TestGoldenFixtureIntegrity::test_golden_fixture_has_twitter_card_tag` — passes against the golden fixture
- `tests/unit/pipeline/test_landing_page.py::TestGenerateLandingHtml` — currently `skipif(not HAS_LANDING_HTML_MODULE)`, will assert Twitter Card tags once Trinity TR-3 lands

## Decision Needed

**Option A:** Add Twitter Card tags to the spec. Update `decisions.md §C` template to include:
```html
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="{{ title }} by {{ author }}">
<meta name="twitter:description" content="{{ translator_note[:160] or title }}">
```
Trinity must implement these in TR-3. Dozer's tests are already written for this.

**Option B:** Drop Twitter Card requirement from the test spec. Dozer will update the golden fixture and remove the Twitter Card assertions from `test_landing_page.py`.

**Recommendation:** Option A. WhatsApp previews (the primary use case per decisions.md) use Open Graph tags, but Twitter/X shares benefit from Twitter Card tags. These are low-cost to add.

## Impact on Trinity

If Option A is chosen, Trinity's TR-3 `generate_landing_html()` implementation must include Twitter Card meta tags. Dozer's contract tests in `TestGenerateLandingHtml` will enforce this automatically once the function is implemented.

---

# Trinity Decision Note: Workspace Pipeline Integration (TR-1 through TR-4)

**Author:** Trinity (Pipeline Dev)  
**Date:** 2026-05-20T23:19:30-04:00  
**Status:** IMPLEMENTED  
**Tasks:** TR-1 (BookWorkspace), TR-2 (metadata.json writer), TR-3 (landing page), TR-4 (pipeline hook)
...
### New files
- `src/transpose/pipeline/workspace.py` — `BookWorkspace`, `build_metadata()`, `generate_landing_html()`, `make_slug()`, `validate_metadata()`, `RightsUnknownViolation`, `PipelineLicenseUpgradeGuard`
- `src/transpose/workspace/landing.py` — public re-export at the path Dozer's tests expected
- `src/transpose/templates/landing.html.j2` — Jinja2 reference template (committed per decisions.md spec)

### Modified files
- `src/transpose/pipeline/runner.py` — Stage 8 `workspace` added; `PipelineOutput.landing_page_url`; `PipelineInput.{source_url, source_edition, translator_note}`; `_run_workspace_stage()` helper
- `src/transpose/config/settings.py` — `blob_static_website_url`, `blob_workspace_container`, `workspace_translator_note`
- `tests/unit/pipeline/test_resume_from.py` — expected stage lists updated to include `workspace`
- `tests/golden/landing_page_fixture.html` — regenerated against new polished HTML implementation
...
### Landing page HTML
- Zero external dependencies (no CDN fonts, no JS, no Bootstrap). All CSS inline.
- Mobile-first, system font stack, warm paper-tone colour palette.
- Two prominent download buttons: "Download Translation" (primary/black) + "Original Scan" (secondary/light).
- `og:type = book`, `twitter:card = summary_large_image`.
- Falls back to `TRANSPOSE_DEFAULT_OG_IMAGE` env var for `og:image` when no per-book cover.
- Does NOT render `license.status` — Shape A pages show title, author, description, download links only.

### Rights-unknown enforcement (Layer 3)
- `build_metadata()` has NO `license_status` parameter. Always sets `rights-unknown`. Final `assert` guards against refactors.
- `write_metadata()` asserts before upload. Raises `RightsUnknownViolation` (not a warning, not a log line).
- `update_metadata()` raises `PipelineLicenseUpgradeGuard` if caller passes a `'license'` key.

### Workspace stage is non-fatal
- If `TRANSPOSE_BLOB_STATIC_WEBSITE_URL` is unset, stage warns and skips.
- Pipeline output artifacts (epub/pdf) are not affected by workspace stage failure.
- Errors are captured in `PipelineOutput.errors` list.

---

## What Tank needs to do before this works end-to-end

1. Complete T-1 (Azure Storage setup) — creates `book-workspaces` container and enables Static Website.
2. Note the static website URL (e.g. `https://transposebooks.z6.web.core.windows.net`) and add `TRANSPOSE_BLOB_STATIC_WEBSITE_URL=<url>` to `.env`.
3. Complete T-2 (DB migration) — `license_status` column on `books` with DEFAULT + CHECK constraint.

---

## What Dozer's tests now cover (all passing)

- `tests/unit/pipeline/test_landing_page.py` — 16 tests including golden snapshot
- `tests/unit/workspace/` — 52 tests including license constraint + metadata schema validation
- Net new: 12 previously-skipped tests now pass (from `not HAS_LANDING_MODULE` → `HAS_LANDING_MODULE`)
